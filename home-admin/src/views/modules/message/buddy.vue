  ...
  <div class="mod-user">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="顾问类型">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.type === 1" size="small" type="danger">AI君</el-tag>
          <el-tag v-else size="small">置业顾问</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        fixed="right"
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="isAuth('sys:user:delete')" type="text" size="small" @click="history(scope.row)">聊天记录
          </el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/sys/api/getConsultantUserList'),
          method: 'get',
          ...
          if (data && data.code === 200) {
            ...
            this.dataList = data.data.list.map(item => {
              return {
                ...item,
                avatar: this.getImageUrl(item.avatar)
              }
            })
          } else {
            this.dataList = []
